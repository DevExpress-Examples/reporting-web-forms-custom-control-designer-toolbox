...
Imports CustomReportControlSample.PredefinedReports

Namespace CustomReportControlSample.Services
    ...
    Public Class CustomReportStorageWebExtension
        ...
        Public Overrides Function IsValidUrl(ByVal url As String) As Boolean
            ...
            Return Equals(Path.GetFileName(url), url)
        End Function
        ...
        Public Overrides Function GetData(ByVal url As String) As Byte()
            ...
            Try
                If Directory.EnumerateFiles(reportDirectory).[Select](New Func(Of String, String)(AddressOf Path.GetFileNameWithoutExtension)).Contains(url) Then
                    Return File.ReadAllBytes(Path.Combine(reportDirectory, url & FileExtension))
                End If

                If Reports.ContainsKey(url) Then
                    Using ms As MemoryStream = New MemoryStream()
                        Call Reports(url)().SaveLayoutToXml(ms)
                        Return ms.ToArray()
                    End Using
                End If
